...
2022-02-11 17:29:25.929 15322-15359/com.github.wlmxenl.exercise.room E/AndroidRuntime: FATAL EXCEPTION: Thread-2
    ...
    java.lang.IllegalStateException: Pre-packaged database has an invalid schema: region_city(com.example.database.room.City).
     Expected:
...
2022-02-11 17:29:25.930 15322-15359/com.github.wlmxenl.exercise.room E/AndroidRuntime:     at androidx.room.RoomDatabase.assertNotSuspendingTransaction(RoomDatabase.java:483)
        ...
        at com.example.database.room.MainActivity.onCreate$lambda-1(MainActivity.kt:29)
        at com.example.database.room.MainActivity.$r8$lambda$Kz5VzJRrVlQB7QeNAuw7J-KgHrM(Unknown Source:0)
        at com.example.database.room.MainActivity$$ExternalSyntheticLambda0.run(Unknown Source:2)
...
if (! _infoRegionCity.equals(_existingRegionCity)) {
  return new RoomOpenHelper.ValidationResult(false, "region_city(com.example.database.room.City).\n"
          + " Expected:\n" + _infoRegionCity + "\n"
          + " Found:\n" + _existingRegionCity);
}
```
